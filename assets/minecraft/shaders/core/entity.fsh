...
vec4 calculateFinalColor(vec4 color) {
    ...
    vec4 fog_color = apply_fog(color, sphericalVertexDistance, cylindricalVertexDistance, FogEnvironmentalStart, FogEnvironmentalEnd, FogRenderDistanceStart, FogRenderDistanceEnd, fogColor);
    #ifdef GLINT
    return vec4(fog_color.rgb, 1.0);
    #else
    return vec4(fog_color.rgb, 0.0);
    #endif
}
